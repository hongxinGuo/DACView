...
TEST(TestCObjectRect, TestGetParaName) {
  CObjectRect c;
  EXPECT_STREQ(_T("Visibility"), c.GetParaName(0));
  EXPECT_STREQ(_T("Height"), c.GetParaName(1));
  EXPECT_STREQ(_T("Width"), c.GetParaName(2));
  EXPECT_STREQ(_T("BackColor"), c.GetParaName(3));
  EXPECT_STREQ(_T("ForeColor"), c.GetParaName(4));
  EXPECT_STREQ(_T("Title"), c.GetParaName(5));
}
