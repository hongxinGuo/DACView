...
namespace DACViewTest {
  TEST(TestCUnitComponentPara, TestInitialize) {
    ...
    EXPECT_STREQ(_T("Para"), c.GetName());
  ...
  TEST(TestCUnitComponentPara, TestSerialize) {
    ...
    EXPECT_STREQ(_T("c1"), pc->GetSrcUnit()->GetName());
    EXPECT_STREQ(_T("UnitAdd2"), pc->GetDestUnit()->GetName());
    EXPECT_STREQ(_T("Name"), pc->GetName());
  ...
  TEST(TestCUnitComponentPara, TestGetSrcUnit) {
    ...
    c.SetSrcUnit(&cp);
    pc = c.GetSrcUnit();
    EXPECT_STREQ(_T("CUnitBase1"), pc->GetName());
  }
  ...
  TEST(TestCUnitComponentPara, TestGetName) {
    CUCPara c;
    c.SetName("CUCP1");
    EXPECT_STREQ(_T("CUCP1"), c.GetName());
  }
