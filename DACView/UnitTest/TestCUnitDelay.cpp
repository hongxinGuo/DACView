...
namespace DACViewTest {
  TEST(TestCUnitDelay, TestInitialize) {
    ...
    EXPECT_STREQ(_T("CUnitDelay"), c.GetName());
    ...
    EXPECT_STREQ(_T("ScanRate"), c.GetParaName(3));
    EXPECT_STREQ(_T(""), c.GetParaName(4));

    EXPECT_STREQ(_T("UnitDelay"), c.GetClassNameStr());
