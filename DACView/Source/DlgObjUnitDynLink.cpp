...
static struct {
  ...
  char* StringName;
  ULONG ulType;
} sm_LinkMethod[MAX_LINK_METHOD_NUMBER] =
...
void CDynamicLinkDlg::UpdateDlg(CObjectDynLink* pcDyn) {
  ...
  if (m_fInputType == FALSE) {  // object -> unit ?
    ...
    SetDlgItemText(IDC_STATIC_UNIT_TO_OBJECT, _T("Object->Unit"));  // update data flow
    ...
    SetDlgItemText(IDC_STATIC_UNIT_TO_OBJECT, _T("Unit->Object"));
