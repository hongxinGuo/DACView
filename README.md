# 简介
DACView是一个组态软件内核，包括了数据的采集、处理和展示。其核心功能—部件（Component），实现了控制的聚合、封装和隔离，使得终端用户构建大型控制系统成为可能且简单明了。
...
2. 部件（Component）。部件。部件是一个可容纳多个部件和简单单元的容器，可无限嵌套。部件可被封装，封装后的部件其外部表现与单元一样，实现了控制的隔离。部件有两种类型，一种允许封装，一种永远不允许封装。
3. 动态链接。单元（或部件）之间通过动态链接进行数据交换。单元部件之间允许相互链接，当从部件链接或者链接至部件时，只允许陷入一层。
4. 部件的封装（Encapsulation）。可封装部件在编译后其内部实现细节被隐藏起来。封装后的部件其外部表现形式与单元完全一样，不再允许动态链接其内部的单元。
